@extends('layout')
@section('title', 'Catálogo de Productos — Rayo Verde')
...
/* ─── Buscador ───────────────────────────────────────── */
.rv-search-bar { display: flex; align-items: center; gap: 12px; margin-bottom: 28px; flex-wrap: wrap; }
.rv-search {
...
.rv-result-count { font-size: 12px; color: var(--ink-muted); font-weight: 500; white-space: nowrap; }
...
/* Imagen protagonista — cuadrada perfecta */
.rv-card-img {
...
.rv-card-precio small {
    font-family: 'Sora', sans-serif; font-size: 12px;
    color: var(--ink-muted); font-weight: 400; margin-left: 2px;
}
...
.rv-stock-text { font-size: 12px; font-weight: 600; letter-spacing: 0.2px; }
...
.sc-low .rv-stock-dot  { background: var(--stock-low); box-shadow: 0 0 5px rgba(192,57,43,0.5); animation: pulso 1.2s ease-in-out infinite; }
...
/* ─── Empty state ────────────────────────────────────── */
.rv-empty-state {
    grid-column: 1 / -1; padding: 80px 20px; text-align: center; color: var(--ink-muted);
}
...
<div class="rv-hero">
    ...
    <div class="rv-stats">
        ...
        <div class="rv-stat">
            <div class="rv-stat-val">Bs. {{ number_format($productos->min('precio'), 0) }}</div>
        ...
        <div class="rv-card-body">
            ...
            <div class="rv-stock-row {{ $sc }}">
                ...
                <span class="rv-stock-text">
                    ...
                    @elseif($qty == 1)
                        ¡Último disponible!
                    @elseif($qty <= $STOCK_LOW)
                        Solo {{ $qty }} unidades
                    @elseif($qty <= $STOCK_MID)
                        {{ $qty }} unidades disponibles
                    @else
                        {{ $qty }} unidades en stock
                    @endif
                </span>
            </div>
